  ...
  return (
    <section className="painel-fichas painel-gerenciar-fichas">
      ...
      <form id="form-criar-ficha-sessao" className="painel-gerenciar-fichas__form" onSubmit={criarFicha}>
        ...
        <label>Nome da ficha<input maxLength="60" value={nome} onChange={(e) => { setNome(e.target.value); setErroCriacao(""); }} placeholder="Novo agente da sessão" /></label>
        ...
        {erroCriacao ? <output className="painel-gerenciar-fichas__erro">{erroCriacao}</output> : null}
      </form>
